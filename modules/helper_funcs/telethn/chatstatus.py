...
from beataniversebot_compat import DRAGONS
from modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def beat_is_admin(chat_id: int):
    status = False
    bot_me = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if bot_me.id == user.id:
            status = True
            break
    return status
