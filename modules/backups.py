...
# from modules.sql import warns_sql as warnssql
import modules.sql.blacklist_sql as blacklistsql

# from modules.sql import cust_filters_sql as filtersql
# import modules.sql.welcome_sql as welcsql
import modules.sql.locks_sql as locksql
import modules.sql.notes_sql as sql
...
from modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("BeatVerse{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("BeatVerse{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `BeatVerse-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("BeatVerse{}.backup".format(chat_id))  # Cleaning file
